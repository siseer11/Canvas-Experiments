...
const bulletsP = document.querySelector('.bullets p');
let pase = 2;
let bulletPase = 10;
let particles = [];
let canShoot = true;
...
function keyDown(e){
	...
	if(thisKey == 'r' && canShoot && playerAmmo != weaponAmmo){ //if the player press r reload just if the weapon is not fully filled OR if the autoReload is going on
		reload();
	}
...
========================================*/
let weaponAmmo = 10;
let playerAmmo = weaponAmmo;
let reloadTime = 1000;
bulletsP.innerText = playerAmmo + '/' + weaponAmmo;
...
function reload(){
	bulletsP.innerText = 'Reloading..';
	canShoot = false;
	console.log('Reloading...');
	window.setTimeout(function(){
		console.log('Reloaded, you can shoot now!');
		canShoot = true;
		playerAmmo = weaponAmmo;
		bulletsP.innerText = playerAmmo + '/' + weaponAmmo;
	},reloadTime)
}


//graphic repr of the bullets;
